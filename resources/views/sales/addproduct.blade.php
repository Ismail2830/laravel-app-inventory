                        ...
                        <form method="post" action="{{ route('sales.product.store', $sale) }}" autocomplete="off">
                            ...
                            <div class="pl-lg-4">
                                    ...
                                    <select name="product_id" id="input-product" class="form-select form-control-alternative{{ $errors->has('product_id') ? ' is-invalid' : '' }}" required>
                                        @foreach ($products as $product)
                                            @if($product['id'] == old('product_id'))
                                                <option value="{{$product['id']}}" selected>[{{ $product->category->name }}] {{ $product->name }} - Base price: {{  $product->price }}MAD</option>
                                            @else
                                                <option value="{{$product['id']}}">[{{ $product->category->name }}] {{ $product->name }} - Base price: {{ $product->price }}MAD</option>
                                            @endif
                                        @endforeach
                                    </select>
                                ...
                                <div class="form-group{{ $errors->has('product_id') ? ' has-danger' : '' }}">
                                    ...
                                    <input type="text" name="total_amount" id="input-total" class="form-control form-control-alternative{{ $errors->has('product_id') ? ' is-invalid' : '' }}" value="0 MAD" disabled>
                                    @include('alerts.feedback', ['field' => 'product_id'])
                                </div>
...
@push('js')
    ...
    <script>
        ...
        function updateTotal () {
            input_total.value = (parseInt(input_qty.value) * parseFloat(input_price.value))+" MAD";
        }
    </script>
@endpush